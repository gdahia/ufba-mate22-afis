...
double sqrdist(cv::Point_<T1> a, cv::Point_<T2> b) {
...
int find_best_match(const cv::Point_<QueryType> qry, const std::vector<cv::Point_<LookupType>> & lookup, bool * marked) {
    ...
    for (int i = 0, len = lookup.size(); i < len; i++) {
        if (!marked[i]) {
            double dist = sqrdist<QueryType, LookupType>(qry, lookup[i]);
            if (dist <= min_dist) {
                min_dist = dist;
                ret = i;
            }
...
int find_similarity(const int idx_a1, const int idx_a2, const int idx_b1, const int idx_b2, const std::vector<cv::Point> & minutiae_a, const std::vector<cv::Point> & minutiae_b) {
    ...
    if (sqrdist<int, double>(mrk[idx_b2], ref[idx_a2]) > MAX_DIST)
        return 0;
...
int afis::match(std::vector<cv::Point> & minutiae_a, std::vector<cv::Point> & minutiae_b) {
    int matches = 0;

    if (minutiae_a.size() > 50 || minutiae_b.size() > 50)
        return 0;
